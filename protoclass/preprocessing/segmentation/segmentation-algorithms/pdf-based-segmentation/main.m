...
path_seg = '../../images/seg/pdf-based/';
if ~exist(path_seg, 'dir')
  mkdir(path_seg);
end

% Annotate each file
for file = 1: 197%  : length( content_list )-1
    ...
    if ( content_list( file ).isdir ~= 1 )
        ...
        if ( strcmp(info.Format, 'jpg') )
            % Open the image
            disp( [ 'PDF-based segmentation of image ', content_list( file ).name ] );
            ...
            [ segImg ] = pdfBasedSegmentation( im );
            % Save the image
            imwrite( segImg, [ path_seg, name, '-mask', '.png' ], 'png' );
            close all;
        end
    end
end
