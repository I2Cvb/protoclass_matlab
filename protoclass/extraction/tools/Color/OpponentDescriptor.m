function [out2]=OpponentDescriptor(patches_R,patches_G,patches_B,number_of_bins,smooth_flag,lambda, patch_size)
...
temsize = patch_size;  
patch_in=zeros(temsize,temsize,3);
out2=zeros(number_of_bins,number_of_patches);

for kk=1:number_of_patches

    patch_in(:,:,1)=reshape(patches_R(:,kk),temsize,temsize);
    patch_in(:,:,2)=reshape(patches_G(:,kk),temsize,temsize);
    patch_in(:,:,3)=reshape(patches_B(:,kk),temsize,temsize);
